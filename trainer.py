...
def visualize_time_series_predictions_and_save(target_series, predicted_series, predictions_dir, epoch):
    ...
    for i in range(len(target_series)):
        ...
        # Save the figure
        plt.savefig(os.path.join(predictions_dir, f'prediction_epoch_{epoch:03d}_batch_{i}.png'))
        plt.close()
...
def train_model(model, criterion, train_loader, validation_loader, start_epoch, num_epochs, best_loss, best_model_path, learning_rate, saved_models_dir, predictions_dir):
    ...
    for epoch in tqdm(range(start_epoch + 1, num_epochs + 1)):
        ...
        visualize_time_series_predictions_and_save(target_series, predicted_series, predictions_dir, epoch)
